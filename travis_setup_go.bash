itravis_setup_go() {
  set -x  # Włączenie wyświetlania szczegółowego wykonywania poleceń
  ...
  go install "golang.org/dl/go${go_version}@latest" --echo
  "go${go_version}" download --echo
  sudo ln -s "${TRAVIS_HOME}/gopath/bin/go${go_version}" "${TRAVIS_HOME}/gopath/bin/go" --echo
